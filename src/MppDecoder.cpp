...
int MppDecoder::AllocateExternalBuffers(size_t buf_size, int width, int height) 
{
    ...
    for (int i = 0; i < 24; i++) {
        ...
        ext_buf.rga_handle = importbuffer_fd(ext_buf.src_dma_fd, buf_size);//将外部内存的物理地址信息映射到RGA驱动内部
        // 提前包装成 RGA 的图像结构体 (NV12 格式)
        // 注意：硬件一般对齐为 256，这里必须使用 m_hor_stride 和 m_ver_stride
        ext_buf.rga_buf = wrapbuffer_handle(ext_buf.rga_handle, width, height, RK_FORMAT_YCbCr_420_SP);//执行RGA图像操作前，要先将输入输出的图像参数转化为统一的 rga_buffer_t 结构作为user API的输入参数

        if (ext_buf.rga_handle <= 0) 
        {
            std::cerr << "RGA importbuffer_fd 失败!" << std::endl;
            dma_buf_free(ext_buf.buf_size, &ext_buf.src_dma_fd, ext_buf.src_buf);
            return -1;
        } //此处失败要注意会有问题
    ...
    while (true) {
        ...
        if (ret == MPP_OK && frame != nullptr) 
        {
            ...
            if (mpp_frame_get_info_change(frame)) 
            {
                ...
                {
                    AllocateExternalBuffers(mpp_frame_get_buf_size(frame), m_hor_stride, m_ver_stride); //内存分配
                }
                
                m_mpi->control(m_ctx, MPP_DEC_SET_EXT_BUF_GROUP, m_frm_grp);
                m_mpi->control(m_ctx, MPP_DEC_SET_INFO_CHANGE_READY, nullptr);//设置外部内存管理以及帧信息改变
                mpp_frame_deinit(&frame);
                frame = nullptr; //这一帧也可以释放掉
            } 
